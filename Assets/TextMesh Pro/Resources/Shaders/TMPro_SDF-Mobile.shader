	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			pixel_t output = {
				...
				half4(vert.xy-_MaskCoord.xy, .5/pixelSize),
			#if (UNDERLAY_ON | UNDERLAY_INNER)
		...
		#if MASK_HARD
			half2 m = 1-saturate(max((abs(input.mask.xy)-_MaskCoord.zw)*input.mask.zw, 0));
			c *= m.x*m.y;
		#endif

		#if MASK_SOFT
			half2 s = half2(_MaskSoftnessX, _MaskSoftnessY);
			half2 m = 1-saturate((max((abs(input.mask.xy)-_MaskCoord.zw)*input.mask.zw+s, 0)) / (1+s));
			c *= m.x*m.y;
		#endif
