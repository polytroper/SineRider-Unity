...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			scale = lerp(scale*(1 - _PerspectiveFilter), scale, abs(dot(input.normal.xyz, normalize(ObjSpaceViewDir(vert)))));
			...
			pixel_t output = {
				...
				float4(vert.xy - _MaskCoord.xy, .5 / pixelSize),
			#if (UNDERLAY_ON || UNDERLAY_INNER)
		...
		#if MASK_HARD
			float2 m = 1-saturate(max((abs(input.mask.xy)-_MaskCoord.zw)*input.mask.zw, 0));
			faceColor *= m.x*m.y;
		#endif

		#if MASK_SOFT
			float2 s = float2(_MaskSoftnessX, _MaskSoftnessX);
			float2 m = 1-saturate((max((abs(input.mask.xy)-_MaskCoord.zw)*input.mask.zw+s, 0)) / (1+s));
			faceColor *= m.x*m.y;
		#endif
